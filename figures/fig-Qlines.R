...
Qlines1 = function(h,ess,u,queue,qh=1/2) {
  ret = matrix(0,length(ess),length(queue))
...
Qlines2 = function(h,ess,u,queue,ph=1/2) {
  ret = matrix(0,length(ess),length(queue))
  for(i in seq_along(queue)) {
    Q = queue[i]
    for(j in seq_along(ess)) {
...
for(page in seq_along(aitch)) {
	...
	hl = Qlines1(h,ess,u,rev(queue),1/2)
	...
	hl = Qlines2(h,ess,u,rev(queue),1/2)
	matplot(log10(hl),log10(ess/hl),type="l",col=rev(bcz),lty=1,lwd=2,
	  xlab="m",ylab="s/m",xaxt="n",yaxt="n",xlim=c(-6,0),ylim=c(0,2)
	)
	...
	hl = Qlines1(h,ess,u,rev(queue),0.99)
	matplot(log10(hl),log10(ess/hl),type="l",col=rev(bcz),lty=1,lwd=2,
	...
	hl = Qlines2(h,ess,u,rev(queue),0.99)
	matplot(log10(hl),log10(ess/hl),type="l",col=rev(bcz),lty=1,lwd=2,
	  xlab="m",ylab="s/m",,xaxt="n",yaxt="n",xlim=c(-6,0),ylim=c(1.5,5)
	)
