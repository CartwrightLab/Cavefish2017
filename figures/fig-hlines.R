...
# This figure calculates how strong selection must be for blindness to become fixed
# in a cave population. Each line represents a different value of dominance.
...
# helper function to handle exported mathematic function
...
# calculate the determinant of delta-q
...
# threshold for the "blindness" allele to have a frequency above qh.
hlines1 = function(aitch,ess,u,Q,qh=1/2) {
  ret = matrix(0,length(ess),length(aitch))
  for(i in seq_along(aitch)) {
    h = aitch[i]
...
# threshold for the average blindness phenotype to be greater than ph.
hlines2 = function(aitch,ess,u,Q,ph=1/2) {
  ret = matrix(0,length(ess),length(aitch))
  for(i in seq_along(aitch)) {
    h = aitch[i]
    for(j in seq_along(ess)) {
...
# each page in the pdf represents a different value of Q
queue = c(0.01,0.001,0.1)
for(page in seq_along(queue)) {
	Q = queue[page]

	####################################################################################
	# Threshold for 'b' to be most common allele

	hl = hlines1(rev(aitch),ess,u,Q,1/2)
	...
	hl = hlines2(rev(aitch),ess,u,Q,1/2)
	matplot(log10(hl),log10(ess/hl),type="l",col=rev(bcz),lty=1,lwd=2,
	  xlab="m",ylab="s/m",xaxt="n",yaxt="n",xlim=c(-6,0),ylim=c(0,2)
	)
	...
	hl = hlines1(rev(aitch),ess,u,Q,0.99)
	matplot(log10(hl),log10(ess/hl),type="l",col=rev(bcz),lty=1,lwd=2,
	...
	hl = hlines2(rev(aitch),ess,u,Q,0.99)
	matplot(log10(hl),log10(ess/hl),type="l",col=rev(bcz),lty=1,lwd=2,
	  xlab="m",ylab="s/m",,xaxt="n",yaxt="n",xlim=c(-6,0),ylim=c(1.5,5)
	)
